module AwsOrganisations
  class Console < Thor
    ...
    def create(account_name, account_email=nil)
      ...
      orgs = AwsUtils::Organisations.new(profile: AwsOrganisations.config.account_admin_profile)
    ...
    def setup_user(account_id)
      sts = AwsUtils::Sts.new(profile: AwsOrganisations.config.account_admin_profile)

      role_creds = sts.credentials_for(account_id, AwsOrganisations.config.account_access_role, AwsOrganisations.config.sts_external_id)
      iam = AwsUtils::Iam.new(credentials: role_creds)

      output = iam.create_user(AwsOrganisations.config.onboarding_ic, account_id)
    end
    ...
    private
      ...
      def set_email_address(account_name)
        AwsOrganisations.config.email_template.sub("%{ACCOUNT_NAME}", account_name.downcase.gsub(/[\s|_]/, "-"))
      end

      def preset_name(account_name)
         AwsOrganisations.config.account_name_template.sub("%{ACCOUNT_NAME}", account_name)
      end
  end
end
